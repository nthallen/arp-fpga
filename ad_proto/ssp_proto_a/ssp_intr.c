...
static void set_ssp_control( unsigned int val, char *where ) {
  int status;
  sg_mutex_lock();
  status = ssp_ll_control(val);
...
//  GAAA! RST isn't defined.
  ...
  set_ssp_control( 0, "Clearing circuit reset" );
}

void xfr_disable(void) {
  set_ssp_control( 0, "Clearing circuit enable" );
...
void xfr_enable(void) {
  ...
  set_ssp_control( 1, "Setting circuit enable" );
  xfrEnabled = 1;
}
