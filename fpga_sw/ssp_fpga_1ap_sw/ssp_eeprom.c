...
int EE_Read( unsigned addr, unsigned char *rbuf, int n_bytes ) {
  ...
  while ( n_bytes > 0 ) {
    ...
    rv = XIic_SetAddress( &Iic, XII_ADDR_TO_SEND_TYPE, ADDR_24LC04B +
      (addr < EE_BLOCK_SIZE ? 0 : 1));
    check_return( rv, "12", "XIic_SetAddress" );
...
int EE_Init(void) {
  ...
  check_return( rv, "141", "XIic_SelfTest" );
  ...
  rv = XIic_Start(&Iic);
  check_return( rv, "147", "XIic_Start" );
...
int EE_ReadConfig( SSP_EE_Config_t *cfg ) {
  int rv;
  uint16_t check;
  status_set( 1, "12", "EE_ReadConfig" );
  rv = EE_Read( 0, (unsigned char *)cfg, sizeof(SSP_EE_Config_hdr_t));
  if ( check_return( rv, "15", "EE_Read" ) ) return 1;
  if ( cfg->hdr.n_bytes > EE_TOTAL_SIZE ||
       cfg->hdr.n_bytes < offsetof(SSP_EE_Config_t,notes)) {
    status_set( 1, "121", "EE_ReadConfig invalid size" );
    return 1;
  }
...
void EE_DefaultConfig(SSP_EE_Config_t *cfg) {
  cfg->version = 0;
...
#ifndef EE_READ_ONLY
	int EE_WriteConfig( SSP_EE_Config_t *cfg ) {
	  int rv;
	  cfg->hdr.n_bytes = offsetof(SSP_EE_Config_t,notes) + strlen(cfg->notes) + 1;
	  if ( cfg->hdr.n_bytes > EE_TOTAL_SIZE ) {
	    report_error( "16", "EE_WriteConfig config too large" );
	    return 1;
	  }
	  cfg->hdr.checksum =
	    fletcher16( (uint8_t *)&cfg->version, cfg->hdr.n_bytes - sizeof(cfg->hdr));
	  status_set( 1, "13", "EE_WriteConfig" );
	  rv = EE_Write( 0, (unsigned char *)cfg, cfg->hdr.n_bytes );
    return check_return( rv, "161", "EE_Write" );
	}
#endif
void EE_print_config( SSP_EE_Config_t *cfg, char *heading ) {
	#if PRINT_ENABLE
  print_mutex_lock();
  safe_printf(("%s\r\n", heading));
